...
impl ReadUntilClient {
    ...
    pub async fn run_cached(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        log::info!("Init action: {:#?}", init_action);
        ...
        let mut dori_stream = self.dori.client.cache(dori_request).await?.into_inner();
        ...
        dori_tx.send(StreamfishRequest { channel: 0, number: 0, id: String::new(), data: Vec::new(), request: init_request })?;
